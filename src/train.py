...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--log_dir", type=str, default=None, help="Log directory (env: TRAIN_LOG_PATH)"
    )
...
def main() -> None:
    ...
    if args.ns_groups_json and os.path.exists(args.ns_groups_json):
        ...
        logging.info(
            f"User NS groups ({len(user_ns_groups)}): {list(ns_groups_cfg['user_ns_groups'].keys())}"
        )
        logging.info(
            f"Item NS groups ({len(item_ns_groups)}): {list(ns_groups_cfg['item_ns_groups'].keys())}"
        )
    else:
        logging.info(
    ...
    logging.info(
        f"PCVRHyFormer model created: num_ns={num_ns}, T={T}, d_model={args.d_model}, rank_mixer_mode={args.rank_mixer_mode}"
    )
